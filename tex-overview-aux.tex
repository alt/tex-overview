...
\makeatletter
%\let\topdfescape\pdf@escapestring  %% to avoid a bug caused by unmatched parenthesis ()
\let\topdfescape\relax
\makeatother
...
%% test if we are in the short or full view
  ...
  %% short view
    \tl_if_eq:NNT#1{\vip}{   %% then check if this is an important node
      \node[coolnode]
	(#2) at (#3) {
        \cooltooltip{#2}{\topdfescape{#4}}{\topdfescape{#4}}{}{#5\strut}
      };
    }
  }{
  %% long view
    \node[coolnode] (#2) at (#3) {
      \cooltooltip{#2}{\topdfescape{#4}}{{#4}}{}{#5\strut}
    };
