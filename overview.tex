...
%%% BIBTEX %%%
\begin{tikzpicture}
	\mynode{bibtex}{0,0}{A helper program to generate a bibliography list.}{\BibTeX};
	
	\mynode{bibtex8}{0,-2}{The documentation says: „An 8-bit Implementation of BibTeX 0.99 with a Very Large Capacity“}{\BibTeX8};
	\draw (bibtex8) to (bibtex);
	
	\mynode{bibtexu}{0,-4}{A unicode-aware version of BibTeX}{\BibTeX u};
	\draw (bibtex8) to (bibtexu);
	
	\mynode{biber}{0,-6}{A cooler version of unicode-aware BibTeX, designed for (?) use with BibLaTeX}{biber};
	\draw (bibtexu) to (biber);
	
	\mynode{biblatex}{3,-6}{A LaTeX package as frontend for biber.}{Bib\LaTeX};
...
\begin{tikzpicture}
	\mynode{dvipdf}{0,0}{Converts dvi files to pdf files.}{dvipdf};

	\mynode{dvipdfm}{0,-2}{Converts dvi files to pdf files. Does not build on dvipdf, but is an independent implementation.}{dvipdfm};

	\mynode{dvipdfmx}{0,-4}{Extended version of dvipdfm. Support for multi-byte encodings and more pdfTeX features. Still active. Combined work of dvipdfm-jpn and dvipdfm-kor.}{dvipdfmx};
	\draw (dvipdfm) to (dvipdfmx);
	
	\mynode{xdvipdfmx}{0,-6}{Converts xdvi files produced by XeTeX to pdf files. Normally always executed after a XeTeX run, so the user won’t notice that an xdvi document was created in between.}{xdvipdfmx};
