...
\overviewsubsection[dvipdfm]{dvipdfm}
\begin{tikzpicture}
	\mynode{dvipdfm}{0,0}{Converts dvi files to pdf files. So far no idea what the „x“ is good for.}{dvipdfm(x)};
	
	\mynode{xdvipdfmx}{0,-2}{Converts xdvi files produced by XeTeX to pdf files. Normally always executed after a XeTeX run, so the user won’t notice that an xdvi document was created in between.}{xdvipdfmx};
	
	\draw (dvipdfm) to (xdvipdfmx);
\end{tikzpicture}
