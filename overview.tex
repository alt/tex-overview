...
\def\overviewsubsection#1{\subsection{#1}\AtEndDocument{\subsection{#1}}}
...
\section{Tree View}
\newpage
\overviewsubsection{\TeX\ -- the program, and extensions/derivatives}
\label{sec:tree}
\Large
\centering
\begin{tikzpicture}
...
\steplayer[-0.5]
\mynode{ant}{13,\layer}{Ant is Not TeX. A typesetting system inspired by TeX. Only *inspired*, so it has nothing to do with TeX in terms of common code.}{ANT};
...
\mynode{vtex}{3.6,\layer}{VTeX (only for linux/OS 2) can produce pdf directly from input. In contrast to pdfTeX, it includes a full PostScript interpreter, thus capable to include EPS figures, PStricks etc.}{V\TeX};
\draw (etex) to (vtex);

\steplayer[-2]
\mynode{aleph}{1,\layer}{originally named epsilon-Omega, an attempt to stabilize Omega while merging epsilon extensions.}{$\aleph$ (\Aleph)};
\draw (omega) to (aleph);
\draw (etex) to (aleph);
...
\newpage
\overviewsubsection{\LaTeX\ (Lamport's \TeX) -- a format and large macro package for \TeX}
\begin{tikzpicture}
\setlayer{2}
...
\steplayer[-1.5]
\mynode{latex091}{-3.5,\layer}{Version 0.91 for use with TeX 0.97 (C) 1983 by Leslie Lamport. Most changes to previous version are in the file lplain.tex.}{\LaTeX\ 0.91};
\draw (latex090) to (latex091);

\steplayer[-0.3]
\mynode{latex092}{0,\layer}{First version with the @ as letter for internal names. Seemlingy first version with a manual. For use with TeX Version 0.999999. (no joke, that's the version number given in the latex.tex file!) (C) 1983 by Leslie Lamport, conversion to 0.92 from 0.91 by Arthur Keller.}{\LaTeX\ 0.92};
\draw (latex091) to (latex092);

\steplayer[-0.3]
\mynode{latex09210}{4,\layer}{Adaption of 0.92 for TeX version 1.0. (C) 1983 by Leslie Lamport, conversion to 0.92 from 0.91 by Arthur Keller.}{\LaTeX\ 0.92 - 1.0};
\draw (latex092) to (latex09210);

\steplayer[-1.5]
\mynode{latex2010}{0,\layer}{Seemingly heavy changes compared to 0.92. Version for TeX 1.0. Release of 11 Dec 1983. No official version 1.x }{\LaTeX\ 2.0 - 1.0};
...
\newpage
\overviewsubsection{\ConTeXt\ -- the other major format and \TeX\ macro package}
\begin{tikzpicture}
...
\newpage
\overviewsubsection{\TeX's "little" helpers}
...
\bibitem[{v\TeX\ reference}()]{vtex}
{V\TeX\ -- the only reference I found.}
\newblock \url{http://www.tex.ac.uk/tex-archive/systems/vtex}
