...
% this document has version number 0.1a
...
\label{textextview}
	\setlayer0
	\TOnode[\vip](tex)(7,\layer)<born in 1978>{\TeX}
	
	\TOnode[\other](ant)(13,\layer)<Ant is Not TeX. A typesetting system inspired by TeX. Only *inspired*, so it has nothing to do with TeX in terms of common code.>{ANT};
	\todraw[dotted](tex)(ant)
	
	\steplayer[-1.5]
	\TOnode(tex-xet)(3,\layer)<The first extension to TeX, 1987. It was able to typeset in two directions, but only with a mark in the dvi to change the direction.>{\TeX-\XeT}
	\todraw(tex)(tex-xet)

	\TOnode(nihongo)(10,\layer)<A true multibyte extension of TeX. Could handle all Japanese characters in one font.>{Nihongo \TeX}
	\todraw(tex)(nihongo)
	
	\TOnode(jtex)(15,\layer)<An extension of TeX for typesetting Japanese. (1987, Yasuki Saito)>{J\TeX}
	\todraw(tex)(jtex)

	\steplayer[-1.5]
	\TOnode(ptex)(10,\layer)<Extension of Nihongo TeX to enable vertical typesetting. ("p" for "publishing")>{p\TeX}
	\todraw(nihongo)(ptex)
	...
	\TOnode[\experimental](uptex)(11,\layer)<Unicode-aware version of pTeX ("unicode-publishing"-TeX) also modernized from TeX3.>{up\TeX}
	\todraw(tex3)(uptex)
	\todraw(ptex)(uptex)
...
\begin{tikzpicture}
	...
	\TOnode(platex)(-4.6,\layer)<A LaTeX-bundle for the pTeX engine.>{p\LaTeXe}
...
%%% META* %%%
\begin{tikzpicture}
	\TOnode[\program](metafont)(0,0)<The program for creating the fonts originally used by TeX.>{METAFONT};

	\TOnode[\program](metapost)(0,-2)<A graphic generating program inspired by METAFONT. MetaPost can produce PostScript graphics as well as SVG.>{MetaPost};
	\todraw(metapost)(metafont)

	\TOnode[\program](metafun)(0,-4)<"MetaFun is Hans Hagen's extension to (or module for) the MetaPost language.">{MetaFun};
...
%%% BIBTEX %%%
\begin{tikzpicture}
	\TOnode[\program](bibtex)(0,0)<A helper program to generate a bibliography list.>{\BibTeX}
	\steplayer[-1]

	\TOnode[\program](nbibtex)(4,\layer)<"NbibTeX helps authors take better advantage of BibTeX data" says the homepage.>{NbibTeX}
	\todraw(nbibtex)(bibtex)
	\steplayer[-0.5]

	\TOnode[\program](bibtex8)(0,\layer)<The documentation says: „An 8-bit Implementation of BibTeX 0.99 with a Very Large Capacity“>{\BibTeX8}
	\todraw(bibtex8)(bibtex)
	\steplayer
	
	\TOnode[\program](bibtexu)(0,\layer)<A unicode-aware version of BibTeX>{\BibTeX u}
	\todraw(bibtex8)(bibtexu)

	\TOnode[\program](pybtex)(3,\layer)<A python implementation of BibTeX.>{Pybtex}
	\todraw(pybtex)(bibtexu)
	\steplayer
	
	\TOnode[\program](biber)(0,\layer)<A cooler version of unicode-aware BibTeX, designed for (?) use with BibLaTeX>{biber}
	\todraw(bibtexu)(biber)
	
	\TOnode[\package](biblatex)(3,\layer)<A LaTeX package as frontend for biber.>{Bib\LaTeX}
...
\begin{tikzpicture}
	\TOnode[\program](dvipdf)(0,0)<Converts dvi files to pdf files.>{dvipdf};
	\steplayer

	\TOnode[\program](dvipdfm)(0,\layer)<Converts dvi files to pdf files. Does not build on dvipdf, but is an independent implementation.>{dvipdfm};
	\steplayer

	\TOnode[\program](dvipdfmx)(0,\layer)<Extended version of dvipdfm. Support for multi-byte encodings and more pdfTeX features. Still active. Combined work of dvipdfm-jpn and dvipdfm-kor.>{dvipdfmx};
	\todraw(dvipdfm)(dvipdfmx)
	\steplayer

	\TOnode[\program](xdvipdfmx)(0,\layer)<Converts xdvi files produced by XeTeX to pdf files. Normally always executed after a XeTeX run, so the user won’t notice that an xdvi document was created in between.>{xdvipdfmx};
...
\begin{tikzpicture}
	\TOnode[\program](metafont)(0,0)<Fonts based on the METAFONT program.>{metafont};
	\steplayer[-3]

	\TOnode[\program](type1)(-2,\layer)<>{PostScript Type 1};
	\TOnode[\program](truetype)(2,\layer)<>{TrueType};
	\TOnode[\program](freetype)(6,\layer)<>{FreeType};
	\todraw(freetype)(truetype)
	\steplayer[-3]

	\TOnode[\program](gx)(6,\layer)<>{TrueType GX};
	\todraw(truetype)(gx)
	\steplayer[-3]

	\TOnode[\program](opentype)(-2,\layer)<>{OpenType};
	\todraw(truetype)(opentype)
	\todraw(type1)(opentype)

	\TOnode[\program](aat)(2,\layer)<>{AAT};
	\todraw(aat)(gx)
\end{tikzpicture}
...
The references are in order of occurance in the above document. I.\,e. if you want information about Lua\TeX, it will be below e.\,g. $\epsilon$\TeX.
	...
	\tobib{MikTeX project site}<http://miktex.org/>
