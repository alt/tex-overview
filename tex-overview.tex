...
\begin{description}
\item[evince 3.0.2] Shows the document correct and complete. Tested using Linux.

\item[Adobe Reader 9] will show all the information but might hide some text of very long tooltips (at least that's the case on my machine). Also, it draws annoying green boxes around the tooltips which do not belong there.

\item[\TeX works 0.5 r869] The built-in PDF viewer of the \TeX works editor does not break lines of tooltips, therefore long annotations are not shown completely.

\item[okular 0.13] also does not break the lines.

\item[xpdf 3.03] shows only very short tooltips. Most of the information is not visible in the graphs.

\item[gv 3.7.2] shows no tooltips, but the annoying green boxes. (Linux)
\end{description}
...
\tograph*({\tostruct[\TeX]{\TeX\ – the program}}){
	...
	\tonode(ptex)(12,\layer)<Extension of Nihongo TeX to enable vertical typesetting. (“p” for “publishing”)  Distributed as WEB change files. Primary author is D. E. Knuth, latest version (TeX live 2011) is pTeX 3.1415926-p3.2.>{p\TeX}
		\todraw(nihongo)(ptex)
		\todraw(tex3)(ptex)
...
\tograph(\tostruct(1)[META*]{META*}[\program]){
	...
	\tonode(mptopdf)(-3,\layer)<Actually a pdfTeX-generated format, this program can be used to compile metapost code directly into PDF output. Metafun is supported, too.>{mptopdf}
...
\tograph(\tostruct(1)[Work Flow]{Work Flow}[\vip]
{
\flushleft
...
The following list tries to explain what happens if a programm is called by a given name. E.\,g. the command \texttt{latex} starts the PDF\TeX\ engine in DVI mode with the format \LaTeX\,2\raisebox{-.5ex}ε. This will list the names used in the official \TeX~live distribution (which should be the same in MiK\TeX) and only those which are not totally clear by the name itself.

\def\totablesec#1{
  \rowcolor[gray]{0.9}\hspace*{-1em}\Large \strut \large\rmfamily #1 & \\
}

\begin{longtable}{>{\ttfamily\quad\large}ll}
\rowcolor[gray]{0.8}\hspace*{-1em}\LARGE \bfseries program & \LARGE  meaning\\
\tiny \\
\totablesec{engines}
tex & starts \TeX\ with the plain format\\
ptex & starts p\TeX\ with the plain format\\
mltex & starts PDFε-\TeX\ with the plain format and DVI output\\ %% FIXME is this true? …
eptex & starts ε-p\TeX\ with the plain format\\
pdftex & starts PDFε-\TeX\ with the plain format and PDF output\\
etex & starts PDFε-\TeX\ with the plain format and DVI output\\
pdfetex & starts PDFε-\TeX\ with the plain format and PDF output\\
aleph & starts the aleph engine\\
xetex & starts \XeTeX\ with the plain format\\
luatex & starts Lua\TeX\ with the plain format and PDF output\\
dviluatex & starts Lua\TeX\ with the plain format and DVI output\\
\totablesec{plain\TeX}
eplain & starts PDFε-\TeX\ with the eplain format and DVI output\\
csplain & starts PDFε-\TeX\ with the csplain format and DVI output\\
pdfcsplain & starts PDFε-\TeX\ with the csplain format and PDF output\\
\totablesec{\LaTeXe}
latex & starts PDFε-\TeX\ with the \LaTeXe\ format and DVI output\\
pdflatex & starts PDFε-\TeX\ with the \LaTeXe\ format and PDF output\\
lamed & starts Aleph with the Lamed format\\
platex & starts ε-p\TeX\ with the p\LaTeX\ format\\
xelatex & starts \XeTeX\ with the \LaTeXe\ format\\
lualatex & starts Lua\TeX\ with the \LaTeXe\ format and PDF output\\
dvilualatex & starts Lua\TeX\ with the \LaTeXe\ format and DVI output\\
\totablesec{Con\TeX t}
texexec & Con\TeX t-related stuff\\ %% FIXME
context & Con\TeX t-related stuff\\ %% FIXME
mtxrun & Con\TeX t-related stuff\\ %% FIXME
\totablesec{other formats}
amstex & starts PDF\TeX\ with the \AMS\TeX\ format and DVI output\\
xmltex & starts PDF\TeX\ with the XML\TeX\ format \\%%  FIXME: what output? …
\totablesec{metafont}
mf & starts the METAFONT program\\
mp & starts the METAPOST program\\
