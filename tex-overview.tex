...
%%% TEX  %%%
\label{textextview}
\tograph*({\overviewsection[\TeX]{\TeX\ – the program}}){
	\setlayer0
...
\clearpage
%%% LATEX %%%
\tograph*(\overviewsection[\LaTeX]{\LaTeX\ – Lamport's \TeX\ format}){
%\hspace*{-5em}
...
%%% CONTEXT %%%
\clearpage
\tograph*(\overviewsection[\ConTeXt]{\ConTeXt: con\,tex\,t – text with tex}){
...
\clearpage
%%%% formats %%%%
\overviewsection[Other Formats]{Other Formats}
%%% YTeX %%%
\tograph(\overviewsubsection[Y\TeX]{Y\TeX\ – a macro package developed at MIT}[\experimental]){
	\tonode(ytex)(0,0)<No real documentation found.>{Y\TeX}
}

%%% StarTeX %%%
\tograph(\overviewsubsection[Star\TeX]{Star\TeX, Starter's \TeX\ – a format designed to help students with short documents}){
	\tonode(startex)(0,0)<A format using <command> instead of \ command>{Star\TeX}
}

%%% JadeTeX %%%
\tograph(\overviewsubsection[Jade\TeX]{Jade\TeX\ – a macro package for processing Jade/OpenJade output}){
	\tonode(jadetex)(0,0)<feed me!>{Jade\TeX}
}

\clearpage

\overviewsection[Pandora's Box]{Pandora's Box}
\parbox{\textwidth}{\normalsize
...
\overviewsubsection[Distributions]{Distributions}[\distro]
\parbox{\textwidth}{\normalsize
...
\tograph(\overviewsubsubsection[\TeX live]{\TeX live}){
...
\tograph(\overviewsubsubsection[Mik\TeX]{Mik\TeX}){
...
\tograph(\overviewsubsubsection[W32\TeX]{W32\TeX}){
...
\newpage
%%% META* %%%
\tograph(\overviewsubsection[META*]{META*}[\program]){
...
%%% BIBTEX %%%
\tograph(\overviewsubsection[Bib\TeX]{Bib\TeX}){
...
%%% dvipdfm %%%
\tograph(\overviewsubsection[{(x)dvipdf(m)(x)}]{(x)dvipdf(m)(x)}){
...
\tograph(\overviewsubsection[Font Technologies]{Font Techonolgies}){
%\parbox{\textwidth}{\normalsize
...
\restoregeometry
\addtocontents{toc}{\newpage}
\addtocontents{toc}{\vspace*{2.95cm}}  %% indeed, this is ridiculous
...
The references are in order of occurance in the above document. I.\,e. if you want information about Lua\TeX, it will be below e.\,g. $\epsilon$\TeX.
	...
	\tobib{Y\TeX\ on CTAN}<http://tug.ctan.org/cgi-bin/ctanPackageInformation.py?id=ytex>
