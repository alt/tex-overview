...
\tograph*(\tostruct[\LaTeX]{\LaTeX\ – Lamport's \TeX\ format}){
	...
	\tonode[\experimental](latex2x)(-1.8,\layer)<An experimental step towards LaTeX3. LaTeX2x is a normal LaTeX2e, but with expl3 and xpackages compiled in the format. It is *not* intended for everyday use but only for experimenting with LaTeX3.>{\LaTeX2x}
	\todraw(latex2x)(latex2ε)
	\steplayer[-2]


	\tonode[\experimental](latex22)(0,\layer)<Inofficial suggestion by Philipp Stephani on the luaLaTeX list. LaTex2.2 should still be a full LaTeX2e, but with the expl3 bundle in the format.>{\LaTeX2.2}
	\todraw(latex2ε)(latex22)
	\steplayer[-2]
	...
	\steplayer[-3]
	\tonode[\experimental](timeslatex)(-5,\layer)<Information needed.>{times\LaTeX}
}
