...
\section{Engine, Format, Distribution}
...
\item[format] A format is a (large) collection of abbreviations (macros) that make the life easy when working with \TeX. The most commonly used formats are \LaTeX, Con\TeX t and plain\TeX. The latter is a minimal set of macros provided by Don Knuth. Formats can be combined with different engines, exploiting the special abilities of these engines. A format is first a collection of text files, but can be compiled into a binary format that can be read in very fast by \TeX.
...
\item information about e-p\TeX\ and p\BibTeX, as the only documentation I found is japanese. If you speek japanese, I'd be happy about a translation!
\end{itemize}
...
\tograph*({\tostruct[\TeX]{\TeX\ – the program}}){
	...
	\tonode(eptex)(13,\layer)<No sure information found; a pTeX successor with eTeX extensions?>{e-p\TeX}
	\todraw(ptex)(eptex)
