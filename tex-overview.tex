...
{\flushleft The chronological order may not be exact in this graph. I have to work hard on the arrangement to show both chronological order and code dependence, and for now only the code dependence is more-or-less correct.
...
\tograph(\tostruct(1)[META*]{META*}[\program]){
	...
	\tonode[\normalimportant](mptopdf)(-4,\layer)<Actually a pdfTeX-generated format, this program can be used to compile metapost code directly into PDF output. Metafun is supported, too.>{mptopdf}
...
and precision of the internal data types.”>{MegaPost}
	...
	\tonode[\normalimportant]%% to indicate that it is a format rather than a program …
			(metafun)(4,\layer)<“MetaFun is Hans Hagen's extension to (or module for) the MetaPost language.” A format for MetaPost that is useable with ConTeXt.>{MetaFun}
...
\def\totablesec#1{
~ \\[-2ex]
  \rowcolor[gray]{0.9}\hspace*{3em}\Large \strut \large\rmfamily #1 & \\
 ~ \\[-2ex]
}
...
mltex & PDFε-\TeX\ with the plain format and DVI output\\
...
texexec --interface = de & dito, with german interface\\
texexec --xtx & \XeTeX with Con\TeX t MKII format\\
context & Lua\TeX\ with Con\TeX t MKIV format and PDF output\\
context --interface = de & dito, with german interface\\
\totablesec{other formats}
amstex & PDF\TeX\ with the \AMS\TeX\ format and DVI output\\
...
pdfxmltex & PDF\TeX\ with the XML\TeX\ format\\%%  FIXME: what output? …
utf8mex & PDFε-\TeX\ with the UTF8MeX format and DVI output\\
xmltex & PDF\TeX\ with the XML\TeX\ format \\%%  FIXME: what output? …
