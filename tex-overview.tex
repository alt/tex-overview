...
%%% TEX  %%%
\label{textextview}
\tograph*({\todisposition[\TeX]{\TeX\ – the program}}){
	\setlayer0
...
\clearpage
%%% LATEX %%%
\tograph*(\todisposition[\LaTeX]{\LaTeX\ – Lamport's \TeX\ format}){
...
%%% CONTEXT %%%
\clearpage
\tograph*(\todisposition[\ConTeXt]{\ConTeXt: con\,tex\,t – text with tex}){
...
\clearpage
%%%% formats %%%%
\todisposition[Other Formats]{Other Formats}

%%% YTeX %%%
\tograph(\todisposition(1)[Y\TeX]{Y\TeX}[\experimental]){
...
%%% StarTeX %%%
\tograph(\todisposition(1)[Star\TeX]{Star\TeX\ – Starter's \TeX}){
...
%%% JadeTeX %%%
\tograph(\todisposition(1)[Jade\TeX]{Jade\TeX}){
	\tonode(jadetex)(0,0)<A macro package for processing Jade/OpenJade output.>{Jade\TeX}
}

\clearpage

\todisposition[Pandora's Box]{Pandora's Box}
\parbox{\textwidth}{\normalsize
...
\todisposition(1)[Distributions]{Distributions}[\distro]
\parbox{\textwidth}{\normalsize
...
\tograph(\todisposition(2)[\TeX live]{\TeX live}){
...
\tograph(\todisposition(2)[Mik\TeX]{Mik\TeX}){
...
\tograph(\todisposition(2)[\TeX\ collection]{\TeX\ collection}){
...
\tograph(\todisposition(2)[W32\TeX]{W32\TeX}){
...
\newpage
%%% META* %%%
\tograph(\todisposition(1)[META*]{META*}[\program]){
...
%%% BIBTEX %%%
\tograph(\todisposition(1)[Bib\TeX]{Bib\TeX}){
...
%%% dvipdfm %%%
\tograph(\todisposition(1)[{(x)dvipdf(m)(x)}]{(x)dvipdf(m)(x)}){
...
\tograph(\todisposition(1)[Font Technologies]{Font Techonolgies}[\fonttechnology]
...
This section tries to cover the development of font technologies – the most important thing for a typesetting system is it's font mechanism …\\[4ex]}
	...
	\tonode(aat)(2,\layer)<"Apple Advanced Typographie" fonts are succesors of the GX fonts. Only available for Mac OS, too.>{AAT}
	\todraw(aat)(gx)
}
