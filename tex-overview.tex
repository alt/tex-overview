...
\item[{\let\nodecolor\distro \tikz \node[coolnode]{distributions};}] Software bundles that bring \TeX\ and friends to the normal user.
...
\label{textextview}
	...
	\tonode[\vip](etex)(4,\layer)<*the* extension to TeX. Provided by the NTS team as an intermediate project until NTS would be ready. eTex is a full TeX and backward compatible.>{$\varepsilon$-\TeX};
	\todraw(tex--xet)(etex)
	\todraw'(tex3)(etex)
	...
	\tonode(texgx)(9.5,\layer)<GX stands for Graphic eXtension. TeXGX was able to handle this font technology. Only on Mac OS.>{\TeX{}gX};
	\todraw(tex3)(texgx)
...
\begin{tikzpicture}
	...
	\tonode[\experimental](lambda)(-6.5,\layer)<A LaTeX-package for the omega-engine.>{$\Lambda$}
	\todraw(latex209)(lambda)
	\steplayer[-1.5]

	\tonode[\experimental](lamed)(-7,\layer)<A LaTeX-package for the aleph-engine.>{Lamed}
	\todraw(lambda)(lamed)
	...
	\steplayer[-3]
	\tonode[\experimental](latex3)(0,\layer)<The long-time successor of LaTeX2e. It is planned to implement a very elaborate low-level programming language. (Almost done by now.) The expl3-package provides an implemantation that can be used on top of LaTeX2e. Several LaTeX packages already make heavy use of expl3. (As does this very document.)>{\LaTeX{}3};
...
\begin{tikzpicture}
	\tonode[\distro](web2c)(0,\layer)<An Implementation and Distribution of TeX which translates the original WEB sources to a C code.>{Web2c}
	\steplayer

	\tonode[\distro](emtex)(3,\layer)<Eberhard Mattes TeX Distribution for MS-DOS and OS/2.>{em\TeX}
	\todraw.(web2c)(emtex)
	\steplayer

	\tonode[\distro](tetex)(3,\layer)<Maintained by Thomas Esser (hence the te in teTeX) from 1994 to May 2006.>{te\TeX}
	\tonode[\distro](4alltexcd)(-3,\layer)<The (vague) past ... (?)>{4All\TeX CD }
	\todraw.(web2c)(tetex)
	\todraw.(web2c)(4alltexcd)
	\steplayer

	\tonode[\distro](fptex)(3,\layer)<A free TeX distribution for Win32 based on teTeX, by Fabrice Popineau. Still active, provides up-to-date binaries for Windows. Special support for Japanese Typesetting.>{fp\TeX}
	\todraw(fptex)(tetex)
	\steplayer

	\tonode[\distro](xemtex)(4,\layer)<A TeX distribution for Windows, based on fpTeX with XEmacs/AucTeX as IDE for (La)TeX. XemTeX was sponsored by the
French government.>{XEm\TeX}
	\todraw(xemtex)(fptex)

	\tonode[\distro](tlpre2008)(0,\layer)<First version 1996 (UNIX only, later also Windows binaries), and then a long story of ongoing work -- see the detailed documentation for a detailed history.>{\TeX live 1996 – 2007}
	\todraw(tetex)(tlpre2008)
	\todraw(4alltexcd)(tlpre2008)
	\todraw.(web2c)(tlpre2008)
	\steplayer

	\tonode[\distro](tl2008)(0,\layer)<A new package manager and network installer are available. So installation via the net is possible as well as package updates. Missing packages are not installed on-the-fly. The last of the modern machines is added: luaTeX>{\TeX live2008}
	\todraw.(tl2008)(tlpre2008)
	\steplayer

	\tonode[\distro](tl2009)(0,\layer)<Dropped Omega and Lambda. Aleph and Lamed are kept.>{\TeX live2009}
	\todraw(tl2009)(tl2008)
	\steplayer
	...
	\tonode[\distro](tlcontrib)(5,\layer)<An extension of TeXlive that contains packages that TeXlive cannot hold because: not free, binary update, not on CTAN or intermediate release. Useable vie the TeXlive manager.>{TLContrib}
...
\begin{tikzpicture}
	\tonode[\distro](w32tex)(0,\layer)<A distributon to provide binaries for windows.>{W32\TeX}
\end{tikzpicture}
...
%%% META* %%%
\begin{tikzpicture}
	\tonode[\program](metafont)(0,\layer)<The program for creating the fonts originally used by TeX.>{METAFONT};
	\steplayer

	\tonode[\program](metapost)(0,\layer)<A graphic generating program inspired by METAFONT. MetaPost can produce PostScript graphics as well as SVG.>{MetaPost};
	\todraw(metapost)(metafont)
	\steplayer

	\tonode[\program](metafun)(0,\layer)<"MetaFun is Hans Hagen's extension to (or module for) the MetaPost language.">{MetaFun};
...
\begin{tikzpicture}
	...
	\tonode[\program](nbibtex)(4,\layer)<"NbibTeX helps authors take better advantage of BibTeX data" says the homepage.>{NbibTeX}
	\todraw(nbibtex)(bibtex)
	\steplayer[-0.5]

	\tonode[\program](bibtex8)(0,\layer)<The documentation says: "An 8-bit Implementation of BibTeX 0.99 with a Very Large Capacity">{\BibTeX8}
	\todraw(bibtex8)(bibtex)
	\steplayer
	
	\tonode[\program](bibtexu)(0,\layer)<A unicode-aware version of BibTeX>{\BibTeX u}
	\todraw(bibtex8)(bibtexu)

	\tonode[\program](pybtex)(3,\layer)<A python implementation of BibTeX.>{Pybtex}
	\todraw(pybtex)(bibtexu)
	\steplayer
	
	\tonode[\program](biber)(0,\layer)<A cooler version of unicode-aware BibTeX, designed for (?) use with BibLaTeX>{biber}
	\todraw(bibtexu)(biber)
...
\begin{tikzpicture}
	...
	\tonode[\program](dvipdfm)(0,\layer)<Converts dvi files to pdf files. Does not build on dvipdf, but is an independent implementation.>{dvipdfm};
	\steplayer

	\tonode[\program](dvipdfmx)(0,\layer)<Extended version of dvipdfm. Support for multi-byte encodings and more pdfTeX features. Still active. Combined work of dvipdfm-jpn and dvipdfm-kor.>{dvipdfmx};
	\todraw(dvipdfm)(dvipdfmx)
	\steplayer

	\tonode[\program](xdvipdfmx)(0,\layer)<Converts xdvi files produced by XeTeX to pdf files. Normally always executed after a XeTeX run, so the user won't notice that an xdvi document was created in between.>{xdvipdfmx};
...
\begin{tikzpicture}
	...
	\tonode[\program](gx)(6,\layer)<Only available for Mac OS.>{TrueType GX};
	\todraw(truetype)(gx)
	\steplayer[-3]
	...
	\tonode[\program](aat)(2,\layer)<Only available for Mac OS.>{AAT};
	\todraw(aat)(gx)
\end{tikzpicture}
...
\normalsize
\obeylines
\flushleft  %% to get rid of the still active centering
...
The references are in order of occurance in the above document. I.\,e. if you want information about Lua\TeX, it will be below e.\,g. $\epsilon$\TeX.
	...
	\tobibsection{Web Sources}
	\tobib{This document on github}<http://github.com/alt/tex-overview>

	\tobibsection{Original Documentation – Engines}
	...
	\tobibsection{Original Documentation – Makro Packages/Formats}
	...
	\tobibsection{Original Documentation – Distributions}
	...
	\tobibsection{Original Documentation – Fonts}
	\tobib{Specifications of the Type1 Fonts}<http://partners.adobe.com/public/developer/en/font/T1_SPEC.PDF>
